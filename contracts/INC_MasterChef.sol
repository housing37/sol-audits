...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of INC
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accIncPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accIncPerShare` (and `lastRewardBlock`) gets updated.
    ...
    Incentive public inc;

    // INC tokens created per second.
    uint256 public incPerSecond;

    // set a max INC per second, which can never be higher than 1 per second
    uint256 public constant maxIncPerSecond = 1e18;
    ...
    constructor(
        Incentive _inc,
        uint256 _incPerSecond,
        uint256 _startTime
    ) {
        inc = _inc;
        incPerSecond = _incPerSecond;
    ...
    function setIncPerSecond(uint256 _incPerSecond) external onlyOwner {
        require(_incPerSecond <= maxIncPerSecond, "setIncPerSecond: too many INC!");
    ...
    function pendingInc(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accIncPerShare = pool.accIncPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 incReward = multiplier.mul(incPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accIncPerShare = accIncPerShare.add(incReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accIncPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accIncPerShare = pool.accIncPerShare.add(incReward.mul(1e12).div(lpSupply));
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 pending = user.amount.mul(pool.accIncPerShare).div(1e12).sub(user.rewardDebt);

        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accIncPerShare).div(1e12);
    ...
    // Safe INC transfer function, just in case if rounding error causes pool to not have enough INC.
    function safeIncTransfer(address _to, uint256 _amount) internal {
        uint256 incBal = inc.balanceOf(address(this));
        if (_amount > incBal) {
            inc.transfer(_to, incBal);
        } else {
            inc.transfer(_to, _amount);
        }
    }
