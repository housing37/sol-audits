// ref: @eightpercent8 @TommyWeb3 request 07.11.24
...
library SwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'aad5517ed2fb887e36f06881141670a655e9ad2c48c5677ffeaad812426cad8d' // init code hash
            ))));
    }
...
interface ISwapRouter01 {
    ...
    function removeLiquidityPLSWithPermit(address token, uint liquidity, uint amountTokenMin, uint amountPLSMin, address to, uint deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s) external returns (uint amountToken, uint amountPLS);
...
interface ISwapRouter02 is ISwapRouter01 {
    function removeLiquidityPLSSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountPLSMin, address to, uint deadline) external returns (uint amountPLS);
    function removeLiquidityPLSWithPermitSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountPLSMin, address to, uint deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s) external returns (uint amountPLS);
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external;
    function swapExactPLSForTokensSupportingFeeOnTransferTokens(uint amountOutMin, address[] calldata path, address to, uint deadline) external payable;
    function swapExactTokensForPLSSupportingFeeOnTransferTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external;
}
...
contract SwapRouter is ISwapRouter02 {
    ...
    function addLiquidityPLS(address token, uint amountTokenDesired, uint amountTokenMin, uint amountPLSMin, address to, uint deadline) external virtual override payable ensure(deadline) returns (uint amountToken, uint amountPLS, uint liquidity) {
        (amountToken, amountPLS) = _addLiquidity(token, WPLS, amountTokenDesired, msg.value, amountTokenMin, amountPLSMin);
        address pair = SwapLibrary.pairFor(factory, token, WPLS);
    ...
    function removeLiquidityPLS(address token, uint liquidity, uint amountTokenMin, uint amountPLSMin, address to, uint deadline) public virtual override ensure(deadline) returns (uint amountToken, uint amountPLS) {
        (amountToken, amountPLS) = removeLiquidity(token, WPLS, liquidity, amountTokenMin, amountPLSMin, address(this), deadline);
    ...
    function removeLiquidityPLSWithPermit(address token, uint liquidity, uint amountTokenMin, uint amountPLSMin, address to, uint deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s) external virtual override returns (uint amountToken, uint amountPLS) {
        address pair = SwapLibrary.pairFor(factory, token, WPLS);
        uint value = approveMax ? uint(-1) : liquidity;
        ISwapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function removeLiquidityPLSSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountPLSMin, address to, uint deadline) public virtual override ensure(deadline) returns (uint amountPLS) {
        (, amountPLS) = removeLiquidity(token, WPLS, liquidity, amountTokenMin, amountPLSMin, address(this), deadline);
        TransferHelper.safeTransfer(token, to, IERC20(token).balanceOf(address(this)));
        IWPLS(WPLS).withdraw(amountPLS);
        TransferHelper.safeTransferPLS(to, amountPLS);
    }

    function removeLiquidityPLSWithPermitSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountPLSMin, address to, uint deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s) external virtual override returns (uint amountPLS) {
        address pair = SwapLibrary.pairFor(factory, token, WPLS);
        uint value = approveMax ? uint(-1) : liquidity;
        ISwapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        amountPLS = removeLiquidityPLSSupportingFeeOnTransferTokens(token, liquidity, amountTokenMin, amountPLSMin, to, deadline);
    }
    ...
    function swapExactPLSForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual override payable ensure(deadline) returns (uint[] memory amounts) {
        require(path[0] == WPLS, 'SwapRouter: INVALID_PATH');
        amounts = SwapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'SwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWPLS(WPLS).deposit{value: amounts[0]}();
        assert(IWPLS(WPLS).transfer(SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }

    function swapTokensForExactPLS(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) returns (uint[] memory amounts) {
        require(path[path.length - 1] == WPLS, 'SwapRouter: INVALID_PATH');
        amounts = SwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'SwapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function swapExactTokensForPLS(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) returns (uint[] memory amounts) {
        require(path[path.length - 1] == WPLS, 'SwapRouter: INVALID_PATH');
        amounts = SwapLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'SwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function swapExactPLSForTokensSupportingFeeOnTransferTokens(uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual override payable ensure(deadline) {
        require(path[0] == WPLS, 'SwapRouter: INVALID_PATH');
        uint amountIn = msg.value;
        IWPLS(WPLS).deposit{value: amountIn}();
        assert(IWPLS(WPLS).transfer(SwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin, 'SwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    }

    function swapExactTokensForPLSSupportingFeeOnTransferTokens(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline) external virtual override ensure(deadline) {
        require(path[path.length - 1] == WPLS, 'SwapRouter: INVALID_PATH');
        TransferHelper.safeTransferFrom(path[0], msg.sender, SwapLibrary.pairFor(factory, path[0], path[1]), amountIn);
